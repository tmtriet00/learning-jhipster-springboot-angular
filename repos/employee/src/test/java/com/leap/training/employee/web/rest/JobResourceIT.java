...
class JobResourceIT {
    ...
    void getAllJobs() throws Exception {
        ...
        restJobMockMvc
            ...
            .andExpect(jsonPath("$.[*].jobId").value(hasItem(job.getJobId().intValue())))
    ...
    void getJob() throws Exception {
        ...
        // Get the job
        restJobMockMvc
            .perform(get(ENTITY_API_URL_ID, job.getJobId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.jobId").value(job.getJobId().intValue()))
    ...
    void putNewJob() throws Exception {
        ...
        // Update the job
        Job updatedJob = jobRepository.findById(job.getJobId()).get();
        ...
        restJobMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedJob.getJobId())
    ...
    void putNonExistingJob() throws Exception {
        int databaseSizeBeforeUpdate = jobRepository.findAll().size();
        job.setJobId(count.incrementAndGet());

        // If the entity doesn't have an ID, it will throw BadRequestAlertException
        restJobMockMvc
            .perform(
                put(ENTITY_API_URL_ID, job.getJobId())
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    void partialUpdateJobWithPatch() throws Exception {
        ...
        partialUpdatedJob.setJobId(job.getJobId());

        partialUpdatedJob.minSalary(UPDATED_MIN_SALARY);

        restJobMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedJob.getJobId())
    ...
    void patchNonExistingJob() throws Exception {
        ...
        restJobMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, job.getJobId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(job))
            )
            .andExpect(status().isBadRequest());
    ...
    void deleteJob() throws Exception {
        ...
        // Delete the job
        restJobMockMvc
            .perform(delete(ENTITY_API_URL_ID, job.getJobId()).accept(MediaType.APPLICATION_JSON))
